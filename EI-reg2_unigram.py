...
#test_file = "2018-EI-reg-En-test-full-sadness.csv"
...
def main():
    ...
    download_dir = "Final-train-dataset-all-short.csv" #where you want the file to be downloaded to 
    csv = open(download_dir, "w") 
    feature= "label"
    for row in range(0,(len(unique_words))+11):
        feature= str(feature) + "," + "name" + str(row)
    row = feature + "\n"
    csv.write(row)
    i=0
    processed_corpus_tfIDF ={} 
    for key, val in unique_unigram_feature.items():
        bigram_list = []
        bigram_list = str(train_label[i]) + ',' + ','.join(str(x) for x in unique_unigram_feature[key]) +',' \
        + str(entropy_value[key])+','+ str(word_frequency[key]) +',' \
    ...
    bigram = find_bigrams(processed_file)
    trygram = find_trygrams(processed_file)
    ...
    for index, row in processed_file.items():
        #print(index,bigram[index])
        c = [new_processed_file[index]] + [[x for x in  bigram[index]]] \
        + [[x for x in  trygram[index]]] + [[entropy_value[index]]] + [[word_frequency[index]]] \
        + pos_features[index] + [[tf_idf_feature[index]]] + [[emo_anger[index]]] + [[emo_joy[index]]] \
        + [[emo_sadness[index]]] + [[emo_fear[index]]] + [[hasTag_Count_anger[index]]] \
        + [[hasTag_Count_joy[index]]] + [[hasTag_Count_sadness[index]]] + [[hasTag_Count_fear[index]]]
    ...
    #training
    download_dir = "Final-dev-dataset-all-short.csv" #where you want the file to be downloaded to 
    csv = open(download_dir, "w") 
    feature= "label"
    for row in range(0,(len(unique_words))+11):
        feature= str(feature) + "," + "name" + str(row)
    ...
    i=0
    processed_corpus_tfIDF ={} 
    for key, val in unique_unigram_feature_dev.items():
        bigram_list = []
        bigram_list = str(dev_label[i]) + ',' + ','.join(str(x) for x in unique_unigram_feature_dev[key]) +',' \
        + str(entropy_value_dev[key])+','+ str(word_frequency_dev[key]) +',' \
    ...
    download_dir = "Final-test-dataset-all-sadness.csv" #where you want the file to be downloaded to 
    csv = open(download_dir, "w") 
    feature= "label"
    for row in range(0,(len(unique_words))+11):
        feature= str(feature) + "," + "name" + str(row)
    ...
    i=0
    processed_corpus_tfIDF ={} 
    for key, val in unique_unigram_feature_test.items():
        bigram_list = []
        bigram_list = str(test_label[i]) + ',' + ','.join(str(x) for x in unique_unigram_feature_test[key]) +',' \
        + str(entropy_value_test[key])+','+ str(word_frequency_test[key]) +',' \
